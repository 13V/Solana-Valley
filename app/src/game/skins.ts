...
// Cosmetic coat recolours for the player's cat farmer.
//
// The character sheet (`pchar`, an 8×24 grid of 48px frames) is almost entirely
// one cream "coat" colour, with a purple ink outline, peach face and yellow
// tuft. A skin is a 3-stop recolour of *just the coat ramp* (highlight / base /
// shadow); the outline, face, tuft and held-tool greys are left untouched, so
// every coat is unmistakably the same cat in a different colour. All 192 frames
// are remapped automatically at boot — we never hand-edit the long sheet.
//
...
// The base sheet's coat colours (packed 0xRRGGBB) → ramp stop index:
// 0 = highlight, 1 = base, 2 = shadow. Every other colour is preserved.
const OUTFIT_STOPS: Array<[number, 0 | 1 | 2]> = [
  [0xf7f7f7, 0],
  [0xf3f4e7, 0],
  [0xf3f2c0, 1], // the dominant cream — the cat's coat
  [0xddd5de, 2],
  [0xdce0d2, 2],
];

// The cat's coats. Classic is free; the rest are coin-sink cosmetics. Order is
// the wardrobe display order (staples first, fantasy colours after).
export const SKINS: Skin[] = [
  { id: 'classic', name: 'Cream', desc: 'The cosy default tabby.', cost: 0, ramp: null, swatch: '#f3f2c0' },
  { id: 'snow', name: 'Snow', desc: 'A crisp pure-white coat.', cost: 500, ramp: [[255, 255, 255], [238, 241, 247], [205, 212, 226]] },
  { id: 'ash', name: 'Ash', desc: 'Classic grey shorthair.', cost: 500, ramp: [[214, 218, 224], [158, 164, 176], [108, 114, 128]] },
  { id: 'midnight', name: 'Midnight', desc: 'A sleek black cat.', cost: 900, ramp: [[96, 100, 118], [54, 57, 72], [32, 34, 46]] },
  { id: 'ginger', name: 'Ginger', desc: 'A warm orange tabby.', cost: 700, ramp: [[255, 221, 170], [240, 160, 86], [200, 112, 52]] },
  { id: 'rose', name: 'Rosie', desc: 'Soft strawberry pink.', cost: 600, ramp: [[255, 224, 230], [244, 150, 175], [206, 110, 140]] },
  { id: 'honey', name: 'Honey', desc: 'Sun-ripened gold.', cost: 1000, ramp: [[255, 240, 196], [240, 205, 100], [196, 150, 55]] },
  { id: 'sky', name: 'Bluebell', desc: 'A storybook blue.', cost: 1200, ramp: [[220, 239, 255], [150, 193, 238], [96, 140, 200]] },
  { id: 'mint', name: 'Sprout', desc: 'Fresh garden green.', cost: 1400, ramp: [[221, 246, 228], [150, 214, 170], [95, 176, 128]] },
  { id: 'lilac', name: 'Lilac', desc: 'Twilight lavender.', cost: 1800, ramp: [[236, 226, 255], [186, 160, 228], [132, 108, 190]] },
];
